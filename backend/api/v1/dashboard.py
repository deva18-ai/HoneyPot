...
async def get_dashboard_stats(
    ...
    unique_sources = await db.execute(
        select(func.count(func.distinct(Incident.source_ip)))
        .where(Incident.first_seen >= start_time)
    )
    
    events_last_hour = await db.execute(
        select(func.count()).where(Event.timestamp >= datetime.now(timezone.utc) - timedelta(hours=1))
    )
    ...
    return DashboardStats(
        active_incidents=status_counts.get("open", 0) + status_counts.get("in_progress", 0),
        ...
        top_classifications=[{"classification": c, "count": cnt} for c, cnt in top_classifications.all()],
...
async def get_threat_overview(
    ...
    kpis["total_events"] = (await db.execute(
        select(func.count()).where(Event.timestamp >= start_time)
    )).scalar()
    
    kpis["high_severity_events"] = (await db.execute(
        select(func.count()).where(and_(Event.timestamp >= start_time, Event.severity == "HIGH"))
    )).scalar()
    
    kpis["unique_ips"] = (await db.execute(
        select(func.count(func.distinct(Event.source_ip))).where(Event.timestamp >= start_time)
    )).scalar()
    
    kpis["active_incidents"] = (await db.execute(
    ...
    kpis["critical_incidents"] = (await db.execute(
        select(func.count()).where(and_(
            Incident.first_seen >= start_time,
    ...
    kpis["alerts_generated"] = (await db.execute(
        select(func.count()).where(Alert.created_at >= start_time)
    )).scalar()
    
    kpis["honeypot_services_active"] = 5
    
    top_attackers = await db.execute(
        select(Event.source_ip, func.count().label("cnt"), func.max(Event.threat_score).label("max_score"))
