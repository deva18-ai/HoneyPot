...
from backend.models import Alert, Event, User
from backend.schemas import AlertResponse, PaginatedResponse
from backend.api.deps import get_current_active_user, require_analyst
...
async def get_alert_stats(
    ...
    recent_critical = await db.execute(
        ...
        .where(and_(
            ...
            Alert.alert_type.in_(["BRUTE_FORCE", "MULTI_SERVICE_SCAN", "HONEYTOKEN_TRIGGERED", "EXPLOIT_ATTEMPT"])
        ))
    ...
    return {
        ...
        "recent_critical": [AlertResponse.model_validate(a) for a in recent_critical.scalars().all()],
