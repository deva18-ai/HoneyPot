...
def _last_hash(db):
    row = db.execute("SELECT event_hash FROM events ORDER BY id DESC LIMIT 1").fetchone()
    return row["event_hash"] if row else None

def log_event(
    source_ip, service, event_type, username=None, password=None,
    request_path=None, payload=None, result=None, session_id=None,
    fingerprint=None, country=None
):
    base = {
        "timestamp": utc_now(), "source_ip": source_ip, "service": service,
        "event_type": event_type, "username": username,
        "password": password, "request_path": request_path,
        "payload": payload, "result": result, "session_id": session_id,
        "fingerprint": fingerprint, "country": country
    }
    ...
    with _LOCK, connect() as db:
        ...
        """, (
            base["timestamp"], source_ip, service, event_type, username, password,
            request_path, payload, result, decision["severity"], session_id,
            fingerprint, country, decision["threat_score"], prev, event_hash
        ))
        ...
        if row:
            ...
            db.execute("""
                ...
                WHERE source_ip=?
            """, (
                1 if result == "FAIL" else 0, services,
                decision["threat_score"], country, base["timestamp"], source_ip
            ))
        else:
            db.execute("""
            ...
            """, (
                source_ip, 1, 1 if result == "FAIL" else 0,
                1, decision["threat_score"], country, base["timestamp"]
            ))
        ...
        text = " ".join(str(base.get(k) or "") for k in ("payload","password","request_path","username"))
        token = db.execute(
