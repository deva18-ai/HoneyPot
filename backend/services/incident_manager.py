...
from backend.models import Incident, IncidentEvent, Event, User
from backend.services.incident import generate_incident_id, calculate_risk_level
...
class IncidentManager:
    ...
    async def create_incident_from_event(
        ...
        async with async_session_maker() as db:
            ...
            if existing:
                return await self._add_event_to_incident(db, existing, event, classification)
    ...
    async def _find_correlated_incident(
        self,
        db,
        event: Event
    ) -> Optional[Incident]:
    ...
    async def _add_event_to_incident(
        ...
        incident.duration_seconds = int((event.timestamp - incident.first_seen).total_seconds())
        incident.threat_score = max(incident.threat_score, classification.get("threat_score", 0))
        incident.risk_level = self._merge_risk_levels(incident.risk_level, classification.get("risk_level", "LOW"))
        
        existing_classifications = set(incident.classification.split("|")) if incident.classification else set()
        new_classifications = set(classification.get("classification", "").split("|"))
        incident.classification = "|".join(existing_classifications | new_classifications)
        
        existing_mitre = set(incident.mitre_techniques.split(",")) if incident.mitre_techniques else set()
    ...
    def _determine_behavior_stage(self, event: Event, classification: Dict[str, Any]) -> str:
    ...
    def _is_key_event(self, event: Event, classification: Dict[str, Any]) -> bool:
        classification_str = classification.get("classification", "")
        key_indicators = [
            "BRUTE_FORCE", "EXPLOIT_ATTEMPT", "HONEYTOKEN_TRIGGERED",
            "SERVICE_SCAN", "PORT_SCAN", "CREDENTIAL_ABUSE"
        ]
    ...
    async def get_incident_with_details(self, incident_id: int) -> Optional[Incident]:
        async with async_session_maker() as db:
            result = await db.execute(
    ...
    async def assign_incident(self, incident_id: int, assignee_id: int) -> Optional[Incident]:
        async with async_session_maker() as db:
            result = await db.execute(select(Incident).where(Incident.id == incident_id))
            incident = result.scalar_one_or_none()
    ...
    async def search_incidents(
        ...
        async with async_session_maker() as db:
            ...
            if filters.get("classification"):
                query = query.where(Incident.classification.ilike(f"%{filters['classification']}%"))
