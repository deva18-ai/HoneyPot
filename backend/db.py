...
def init_db():
    with connect() as db:
        ...
        db.executemany(
            ...
            [("admin","honeytrap-admin","admin"),("viewer","honeytrap-viewer","viewer")]
        )
        tokens = [
...
def stats():
    with connect() as db:
        ...
        high = db.execute("SELECT COUNT(*) c FROM events WHERE severity='HIGH'").fetchone()["c"]
        ips = db.execute("SELECT COUNT(*) c FROM ip_stats").fetchone()["c"]
        by_service = [dict(r) for r in db.execute(
            "SELECT service, COUNT(*) count FROM events GROUP BY service ORDER BY count DESC"
        )]
        top_ips = [dict(r) for r in db.execute(
        ...
        return {
            "total_events": total, "alerts": alerts, "high_severity": high,
            "unique_ips": ips, "by_service": by_service, "top_ips": top_ips
        }
...
def all_events():
    with connect() as db:
        return [dict(r) for r in db.execute("SELECT * FROM events ORDER BY id").fetchall()]
