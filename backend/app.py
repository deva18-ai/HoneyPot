...
from .config import HOST, WEB_PORT, DASHBOARD_DIR, DEMO_USERS
...
def login():
    ...
    if user and secrets.compare_digest(password, user["password"]):
        ...
        log_event(request.remote_addr or "127.0.0.1","HTTP","AUTH_ATTEMPT",
                  username=username,password=password,result="SUCCESS",
                  request_path="/login",payload="dashboard login")
        return jsonify({"ok":True,"role":user["role"]})
    log_event(request.remote_addr or "127.0.0.1","HTTP","AUTH_ATTEMPT",
              username=username,password=password,result="FAIL",
              request_path="/login",payload="dashboard login")
...
def decoy_files():
    ...
    if name not in allowed:
        log_event(ip,"HTTP","PATH_REQUEST",request_path="/decoy/files",
                  payload=name,result="FAIL")
        return "Not found",404
    log_event(ip,"FILE_STORE","FILE_OPEN",request_path=name,
              payload=allowed[name],result="SUCCESS")
